...
class ofApp : public ofBaseApp{
	...
	public:
        ...
        bool walkRight; // 오른쪽 화살표 키 입력을 감지해서 버텍스 셰이더로 쏴주려고 하는, 얼만큼 움직일 지에 대한 값인 charPos 를 계산할 지 말지를 결정할 boolean 변수
        glm::vec3 charPos; // 버텍스 셰이더에 쏴줘서 "캐릭터를 이만큼 이동시켜라" 라고 더해줄 vec3 변수 (멤버변수 선언 시 (0, 0, 0) 으로 초기화된 상태겠지?)
};
